// *****************************
// Binary "Evolutionary" search classes
// *****************************
...
class BSearch {
	public:
		// The constructor
		BSearch(int vectorSize = 0,double (*EvalFn)(TVector<int> &, RandomState &) = NULL);
		// The destructor
		~BSearch();
		// Basic Accessors
    ...
    void SetRandomSeed(long seed) {rs.SetRandomSeed(seed);};
		...
		void SetEvaluationFunction(double (*EvalFn)(TVector<int> &v, RandomState &rs))
			{EvaluationFunction = EvalFn;};
		void SetBestActionFunction(void (*BestFn)(int Generation,TVector<int> &v))
    ...
    //friend ostream& operator<<(ostream& os, BSearch& s);
		//friend istream& operator>>(istream& is, BSearch& s);

	private:
		// Helper Methods
		void DoSearch(int ResumeFlag);
		int EqualVector(TVector<int> &v1, TVector<int> &v2)
		{
		...
		void RandomizeVector(TVector<int> &Vector);
		void RandomizePopulation(void);
		double EvaluateVector(TVector<int> &Vector, RandomState &rs);
    friend void *EvaluatePopulationRange(void *arg);
		...
		void MutateVector(TVector<int> &Vector);
		void UniformCrossover(TVector<int> &v1, TVector<int> &v2);
		void TwoPointCrossover(TVector<int> &v1, TVector<int> &v2);
    ...
    TVector<RandomState> RandomStates;
		...
		TVector<TVector<int> > Population;
		TVector<double> Perf;
		...
		int CheckpointInt;
		// Function Pointers
		double (*EvaluationFunction)(TVector<int> &v, RandomState &rs);
		void (*BestActionFunction)(int Generation,TVector<int> &v);
		...
		void (*SearchResultsDisplayFunction)(BSearch &s);
};


// A range specification structure for threaded evaluation

struct PopRangeSpec {BSearch *search; int start; int end;};
